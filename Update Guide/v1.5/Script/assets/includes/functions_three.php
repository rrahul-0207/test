...
function Wo_GetAds($last_id = 0) {
    ...
    $sql   = "SELECT * FROM  " . T_USER_ADS . " WHERE `id` > 0 {$query_one} ORDER BY `id` DESC LIMIT 15";
    $query = mysqli_query($sqlConnect, $sql);
    $data  = array();
    while ($fetched_data = mysqli_fetch_assoc($query)) {
        $fetched_data['user_data'] = Wo_UserData($fetched_data['user_id']);
...
function Wo_GetAllStories($last_id = 0) {
    ...
    $sql   = "SELECT * FROM  " . T_USER_STORY . " WHERE `id` > 0 {$query_one} ORDER BY `id` DESC LIMIT 15";
    $query = mysqli_query($sqlConnect, $sql);
    $data  = array();
    while ($fetched_data = mysqli_fetch_assoc($query)) {
...
function Wo_RegisterAdConversionClick($id) {
    ...
    $result = false;
    if ($ad && is_array($ad) && !empty($ad) && isset($ad['user_data']) && !Wo_IsConversionExists($id)) {
    ...
    } else if ($ad && is_array($ad) && !empty($ad) && isset($ad['user_data'])) {
...
function Wo_IsStoryOwner($id) {
    ...
    if ($query) {
        $fetched_data = mysqli_fetch_assoc($query);
        if ($fetched_data['user_id'] == $user || Wo_IsAdmin()) {
            $result = true;
        }
    }
    return $result;
...
function Wo_GetStroies($args = array()) {
    ...
    while ($fetched_data = mysqli_fetch_assoc($query)) {
        ...
        $fetched_data['is_owner']  = ($fetched_data['user_id'] == $wo['user']['id'] || Wo_IsAdmin()) ? true : false;
        $data[]                    = $fetched_data;
    }
    return $data;
}
...
function Wo_GetUsersByName($name = '',$friends = false) {
    global $sqlConnect, $wo;
    ...
    $sql     = "SELECT `user_id` FROM " . T_USERS . " WHERE `user_id` <> {$user} AND `username`  LIKE '%$name%' {$sub_sql}";
    $query   = mysqli_query($sqlConnect, $sql);
    while ($fetched_data = mysqli_fetch_assoc($query)) {
        $data[] = Wo_UserData($fetched_data['user_id']);
    }
...
function Wo_FriendPrivacy($user_id = false, $friend_privacy = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$user_id || !is_numeric($friend_privacy)) {
        return false;
    }
    ...
    if ($friend_privacy == 0) {
        ...
        $privacy   = mysqli_num_rows($data_rows) > 0;
...
function Wo_CheckGroupAdminPassword($password = false, $group_id = false) {
    ...
    if (Wo_IsGroupUserExists($user_id, $group_id)) {
        ...
        if (mysqli_num_rows($data) == 1) {
            $fetched_data = mysqli_fetch_assoc($data);
            if ($fetched_data['password'] == $password) {
                $match = true;
            }
        }
    }
...
function Wo_CheckPageAdminPassword($password = false, $page_id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !is_numeric($password) || !is_numeric($page_id)) {
        return false;
    }
    $user_id = Wo_Secure($wo['user']['user_id']);
