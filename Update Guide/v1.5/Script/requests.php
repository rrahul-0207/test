...
if ($f == 'admin_setting' AND (Wo_IsAdmin() || Wo_IsModerator())) {
    ...
    if ($s == 'update_general_setting' && Wo_CheckSession($hash_id) === true) {
        ...
        if (!empty($_POST['user_ads'])) {
            $user_ads = $_POST['user_ads'];
        }
        ...
        if (!empty($_POST['find_friends'])) {
            $find_friends = $_POST['find_friends'];
        }
        $saved_data = array(
    ...
    if ($s == 'delete_reported_content' && Wo_IsAdmin()) {
...
if ($f == 'chat') {
    ...
    if ($s == 'create_group' && isset($_POST['group_name']) && isset($_POST['parts'])) {
        ...
        if (strlen($_POST['group_name']) < 4 || strlen($_POST['group_name']) > 15) {
            $error           = true;
            $data['message'] = $error_icon . 'Group name must be 4/15 characters';
        }
        if (isset($_FILES["avatar"])) {
            if (file_exists($_FILES["avatar"]["tmp_name"])) {
                ...
                if (!in_array($image[2], array(
                    ...
                    $error           = true;
                    $data['message'] = $error_icon . 'Group avatar must be an image';
                }
            }
        }
        if (!$error) {
...
if ($f == 'pages') {
    ...
    if ($s == 'delete_page') {
        if (!empty($_POST['page_id']) && Wo_CheckSession($hash_id) === true) {
            if (md5($_POST['password']) != $wo['user']['password'] && !Wo_CheckPageAdminPassword(md5($_POST['password']), $_POST['page_id'])) {
                $errors[] = $error_icon . $wo['lang']['current_password_mismatch'];
            }
            if (empty($errors)) {
...
if ($f == 'groups') {
    ...
    if ($s == 'delete_group') {
        if (!empty($_POST['group_id']) && Wo_CheckSession($hash_id) === true) {
            if (md5($_POST['password']) != $wo['user']['password'] && !Wo_CheckGroupAdminPassword($_POST['password'], $_POST['group_id'])) {
                $errors[] = $error_icon . $wo['lang']['current_password_mismatch'];
            }
            if (empty($errors)) {
...
if ($f == 're_cover') {
    if (isset($_POST['pos'])) {
        if ($_POST['cover_image'] != $wo['userDefaultCover']  && (Wo_GetMedia($_POST['cover_image']) == $wo['user']['cover'] || Wo_IsAdmin())) {
...
if ($f == "add-new-forum" && Wo_IsAdmin() == true && Wo_CheckMainSession($hash_id) === true) {
...
if ($f == 'ads') {
    if ($s == 'new') {
        if (!isset($_POST['name']) || !isset($_POST['website']) || !isset($_POST['headline']) || !isset($_POST['description']) || !isset($_POST['audience-list']) || !isset($_POST['gender']) || !isset($_POST['bidding']) || !isset($_FILES['image']) || !isset($_POST['location']) || !isset($_POST['appears'])) {
            $error = $error_icon . $wo['lang']['please_check_details'];
        } else {
...
if ($f == 'notifications') {
    if ($s == 'get-users') {
        ...
        if ($users && count($users) > 0) {
            foreach ($users as $wo['notificated-user']) {
                $html .= Wo_LoadPage('admin/send_notification/users-list');
            }
            $data['status'] = 200;
            $data['html']   = $html;
        }
    ...
    if ($s == 'send') {
        ...
        $error = false;
        $users = array();
        if (!isset($_POST['url']) || !isset($_POST['description']) || (!isset($_POST['all']) && !isset($_POST['notifc-users']))) {
            $error = true;
        } else {
...
if ($f == 'admincp') {
    if ($s == 'insert-invitation') {
        ...
        if ($wo['invitation'] && is_array($wo['invitation'])) {
            $data['html']   = Wo_LoadPage('admin/invitations/invitation-list');
            $data['status'] = 200;
        }
        header("Content-type: application/json");
        echo json_encode($data);
