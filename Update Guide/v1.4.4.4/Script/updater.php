...
if ($f == 'run_updater') {
        ...
        if (!empty($_GET['purchase_code'])) {
            ...
            $file = file_get_contents("http://www.wowonder.com/check_for_updates.php?code={$purchase_code}&version=$version", false, stream_context_create($arrContextOptions));
            $check = json_decode($file, true);
            if (!empty($check['status'])) {
                if ($check['status'] == 'SUCCESS') {
                    if (!empty($check['versions'])) {
                        foreach ($check['versions'] as $key => $version) {
                            $updater = file_put_contents('script.zip', file_get_contents("https://www.wowonder.com/get_update.php?code={$purchase_code}&version=$version", false, stream_context_create($arrContextOptions)));
                            if ($updater) {
                                $updater = file_put_contents('theme.zip', file_get_contents("https://www.wowonder.com/get_update.php?code={$purchase_code}&version=$version&theme=true", false, stream_context_create($arrContextOptions)));
                                if ($updater) {
