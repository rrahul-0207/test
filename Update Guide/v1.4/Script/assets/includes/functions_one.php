...
function Wo_CreateLoginSession($user_id = 0) {
    ...
    $query    = mysqli_query($sqlConnect, "DELETE FROM " . T_APP_SESSIONS . " WHERE `user_id` = '{$user_id}' AND `platform` = 'web' AND `time` < $time");
    $user_id  = Wo_Secure($user_id);
    $hash     = Wo_Secure(md5(time()));
    $query_    = mysqli_query($sqlConnect, "INSERT INTO " . T_APP_SESSIONS . " (`user_id`, `session_id`, `platform`, `time`) VALUES('{$user_id}', '{$hash}', 'web', " . time() . ")");
    if ($query_) {
        return $hash;
    } 
}
...
function Wo_GetMessagesHeader($data = array()) {
    ...
    $user_id        = Wo_Secure($data['user_id']);
    if (!empty($data['session_id'])) {
        $logged_user_id = Wo_GetUserFromSessionID($data['session_id']);
...
function Wo_RegisterMessage($ms_data = array(), $session_id = 0) {
    ...
    if (empty($session_id)) {
        ...
        if (Wo_GetUserFromSessionID($session_id) == 0) {
            return false;
        }
    }
    if (empty($ms_data)) {
    ...
    if (move_uploaded_file($data['file'], $filename)) {
        if ($second_file == 'jpg' || $second_file == 'jpeg' || $second_file == 'png' || $second_file == 'gif') {
            if ($type == 1) {
                ...
                @Wo_Resize_Crop_Image(400, 400, $filename, $last_file, 60);
            } else {
                if ($second_file != 'gif') {
                    @Wo_CompressImage($filename, $filename, 50);
                }
