...
$(document).on('keydown', '.postText', function(event) {
  $('.postText').autogrow({vertical: true, horizontal: false, height: 200});
});
$(document).on('keydown', '#movies-comment', function(event) {
  $('#movies-comment').autogrow({vertical: true, horizontal: false, height: 200});
});
$(document).on('keydown', '#blog-comment', function(event) {
...
function Wo_LoadStory(type ='all',user_id = false) {
...
// open chat tab
function Wo_OpenChatTab(recipient_id, group_id = 0) {
...
function Wo_HidePost(post_id = false){
  if (!post_id) {
    return false;
  }
...
function Wo_SharePost(post_id = false,owner_id = false,self){
...
function Wo_AddGroupUserAdmin(member_id = false,group_id = false,self = false){
...
function Wo_OpenLighteBox(self = false,event = false){
  if (!self || !event) {
    return false;
  }
...
function Wo_UpdateLocation(position = false) {
  if (!position) {
    return false; 
  }
