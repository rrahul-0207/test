...
function Wo_FriendPrivacy($user_id = false, $friend_privacy = false) {
    global $sqlConnect, $wo;
    if (!$user_id || !is_numeric($friend_privacy)) {
        return false;
    }
    if ($wo['loggedin'] == true) {
    ...
    while ($fetched_data = mysqli_fetch_assoc($query_run)) {
        ...
        if (empty($fetched_data['thumbnail'])) {
            ...
            if (empty($fetched_data['thumb'])) {
                $fetched_data['thumb']['filename'] = $fetched_data['thumb'];
            }
            if (empty($fetched_data['thumb'])) {
                $fetched_data['thumb']['filename'] = $fetched_data['user_data']['avatar'];
            }
        } else {
            $fetched_data['thumb'] = array();
            $fetched_data['thumb']['filename'] = Wo_GetMedia($fetched_data['thumbnail']);
        }
        $fetched_data['videos']    = Wo_GetStoryMedia($fetched_data['id'], 'video');
        $fetched_data['is_owner']  = ($fetched_data['user_id'] == $wo['user']['id'] || Wo_IsAdmin()) ? true : false;
        $data[] = $fetched_data;
