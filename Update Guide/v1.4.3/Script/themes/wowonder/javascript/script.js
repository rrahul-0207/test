...
function Wo_AnswerCall(id, url, type = 'video') {
  Wo_progressIconLoader($('#re-calling-modal').find('.answer-call'));
  $.get(Wo_Ajax_Requests_File(), {f:'answer_call', id:id, type:type}, function (data) {
    Wo_PlayVideoCall('stop');
    if (type == 'video') {
      if (data.status == 200) {
...
function Wo_DeclineCall(id, url, type = 'video') {
  Wo_progressIconLoader($('#re-calling-modal').find('.decline-call'));
  $.get(Wo_Ajax_Requests_File(), {f:'decline_call', id:id, type:type}, function (data) {
    if (data.status == 200) {
      Wo_PlayVideoCall('stop');
      $( '#re-calling-modal' ).remove();
      $( '.modal-backdrop' ).remove();
