...
function Wo_GetAvUpgrades($user_id) {
      ...
      if ($user['pro_type'] == 1) {
            $can_see = array('month' => $hot_member, 'year' => $ultima_member, 'life-time' => $vip_member);
      } else
      if ($user['pro_type'] == 2) {
            $can_see = array('year' => $ultima_member, 'life-time' => $vip_member);
      } else
      if ($user['pro_type'] == 3) {
            $can_see = array('life-time' => $vip_member);
      } else
      if ($user['pro_type'] == 4) {
            $can_see = array();
      }
...
function Wo_GetProPackages() {
    ...
    $data = array(
        'free' => array('id' => 0, 'name' => $free_member),
        'star' => array('id' => 1, 'name' => $star_member),
        'hot' => array('id' => 2, 'name' => $hot_member),
        'ultima' => array('id' => 3, 'name' => $ultima_member),
        'vip' => array('id' => 4, 'name' => $vip_member)
    );
    return $data;
}
...
function Wo_CheckFroInCalls() {
    ...
    $data1 = array();
    $time  = time() - 40;
    $query  = mysqli_query($sqlConnect, "SELECT * FROM " . T_VIDEOS_CALLES . "  WHERE `to_id` = '{$user_id}' AND `time` > '$time' AND `active` = '0' AND `declined` = 0");
    if (mysqli_num_rows($query) > 0) {
        $sql = mysqli_fetch_assoc($query);
...
function Wo_GetProfileFields($type = 'all') {
    ...
    $data      = array();
    $where     = '';
    $placements = array('profile', 'general', 'social');
