...
function Wo_RegisterCommentKeyDown(text, post_id, user_id, event, page_id, type, visibilityDrpDwnVal) {
    if(text.trim() != '' && text.trim() != null && event.keyCode == 13 && event.shiftKey == 0) {
            Wo_RegisterCommentClick2(text, post_id, user_id, page_id,visibilityDrpDwnVal);
    }
}
function Wo_RegisterCommentClick2(text, post_id, user_id, page_id,visibilityDrpDwnVal){
    Wo_RegisterCommentClick(text, post_id, user_id, page_id,'',visibilityDrpDwnVal);
}
// register post comment
function Wo_RegisterCommentClick(text, post_id, user_id, page_id, type,visibilityDrpDwnVal) {
    
    var prvcDrpDwnVal = document.getElementById(visibilityDrpDwnVal).value;
    var c_image = $('#comment_src_image').val();
    post_wrapper = $('[id=post-' + post_id + ']');
    comment_textarea = post_wrapper.find('.post-comments');
    comment_btn = comment_textarea.find('.emo-comment');
    textarea_wrapper = comment_textarea.find('.textarea');
    //c_image=$('#comment_src_image').val();
   // alert(c_image);
  ...
  $.ajax({
        ...
        success: function (data) {
          if (data.status == 200) {
            ...
            $('#cmt_img').append('<input type="hidden" value="' + data.image_src + '" id="comment_src_image" name="comment_image" />');
